...
[JJ_emodps, Popt, JJ_int] = run_emodps(pClass, moea_param) ;

figure; plot( JJ_emodps(:,1), JJ_emodps(:,2), 'o' );
hold on
% Get objective values every 10 generations
for i=1:floor(moea_param.gen/10-1E-3)
    plot(JJ_int(10*i,1:2:end), JJ_int(10*i,2:2:end),'x')
end

xlabel('flooding (cm)'); ylabel('irrigation deficit (m3/s)');
