...
function flows = water_balance_inter(reservoir, flows)
...
for t =1:T
    ...
    if head > reservoir.demand_intake_level   
        % At-site withdrawals: meet the demand (if available) or take what's
        % left
        w(t) = min(wa, sum(flows.local_demand(t,:),2)); 
...
flows.storage_inter = s;
flows.release_inter = r;
flows.spillage_inter = l;
flows.withdrawals_inter = w;
